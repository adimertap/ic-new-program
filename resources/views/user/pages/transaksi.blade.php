...
@section('content')
<main class="col-md-12" style="margin-bottom: 80px">
                        ...
                        <tbody>
                            ...
                            <tr role="row" class="odd">
                                ...
                                <td class="text-center text-primary">
                                    ...
                                    <span id="{{ $hargaOtomatis }}">
                                        Rp. {{ convert_to_rupiah($hargaOtomatis) }}(50%)
                                    </span>

                                    @elseif($item->status == 5 && $item->payment_status == 'Cicilan')

                                    <span id="{{ $hargaOtomatis }}">
                                        Rp. {{ convert_to_rupiah($hargaOtomatis) }}
                                        (25%)
                                    </span>

                                    @elseif($item->status == 4 && $item->payment_status == 'Cicilan')

                                    <span id="{{ $hargaOtomatis }}"> Rp.
                                        {{ convert_to_rupiah($hargaOtomatis) }}(75%)
                                    </span>

                                    @elseif($item->status == 2 && $item->payment_status == 'Paid')

                                    <span id="{{ $hargaOtomatis }}">Rp.
                                        {{ convert_to_rupiah($hargaOtomatis) }}
                                        (Lunas)
                                    </span>
                                ...
                                <td class="text-center">
                                    <?php 
                                      $nilaiVoucher = 0;
                                      if($item->Voucher){
                                        $nilaiVoucher = $item->Voucher->nilai;
                                      }

                                      if($item->type_diskon == 'Persen'){
                                        $disc = ($item->harga_kelas/100) * $item->diskon;
                                        $angka_diskon = $item->harga_kelas - ($disc + $nilaiVoucher);
                                      }else {
                                          $angka_diskon = $item->harga_kelas - ($item->diskon + $nilaiVoucher);
                                      }
                ...
                <form action="{{ route('bayar', '1') }}" method="POST" id="formGenerate">
                    ...
                    <div class="modal-body small">
                        ...
                        <div class="form mb-3 mt-3">
                            ...
                            <select class="form-control form-control-sm mb-3" name="cicilan" id="selectCicilan">
                                <option value="Lunas" id="sisaLunas">Lunas</option>
                                <option value="25" id="sisa25" style="display: none">Bayar 25%</option>
                            </select>
                        </div>
                    </div>
...
<script>
    $(document).ready(function() {
        ...
        table.on('click', '.payment', function() {
            ...
            $.ajax({
                url: '/transaksi/hitung/' + id
                , type: "GET"
                , success: function(response) {
                    console.log(response)
                    var nilai = response.fix;
                    var transaksi = response.transaksi;
                    $('#modalBayar').modal('show')
                    $('#sisaPembayaran').val(nilai)
                    $('#transaksi_id').val(id)
                    $('#sisaBayarHidden').val(nilai)
                    $('#id_transaksi').val(id)
            ...
            //     } else {
            //         var percentage = parseFloat(formattedPrice.match(/\((-?\d+(?:\.\d+)?)%\)/)[1]);
            //             formattedPrice = $(formattedPrice).attr('id')
            //         if (percentage == 75) {
            //             var startIndex = sisaBayar.indexOf("Rp.") + 4; // Add 4 to skip "Rp. "
            //             var endIndex = sisaBayar.indexOf("(-75%)");
            //             formattedPrice = sisaBayar.substring(startIndex, endIndex).trim();
            //         } else if (percentage == 50) {
            //             $('#sisa25').prop('disabled', false);
            //             var startIndex = sisaBayar.indexOf("Rp.") + 4; // Add 4 to skip "Rp. "
        ...
        $('#btnCancel').on('click', function() {
            ...
            $('#selectCicilan').val('Lunas').trigger('change');
        })

        $('#selectCicilan').on('change', function(){
            var tenor = $(this).val()
